...
- Align the MCP HTTP endpoint and browser origin with the configured resource URL,
  including nested paths. Reject resource URLs with missing hosts, credentials,
  query strings or fragments before server startup.
- Keep automated MCP dependency updates within the supported v1 major version;
  v2 requires a separately tested adapter migration.
- Refresh the combined governed-service demo to reviewed dependency commits and
  synchronize its documented source references.
...
- Add optional authenticated MCP SDK v1 resource server, trusted RS256 issuer/audience
  verification, per-user scopes, strict tool arguments, permission approval and replay safety.
- Bind illustrative manifest hashes to canonical nested schema, permissions and source.
  Existing expected hashes must be regenerated and independently reapproved.
- Remove tracked generated bytecode/egg-info and replace stale headline test counts with CI.
- SDK v2 is outside this release's tested compatibility range.
